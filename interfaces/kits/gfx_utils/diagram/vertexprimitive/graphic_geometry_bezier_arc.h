...
#include "gfx_utils/diagram/depiction/graphic_depict_transform.h"
...
public:
    ...
    unsigned Vertex(float* x, float* y)
    {
        ...
        *y = arrayVertices_[vertexIndex_ + 1];
        vertexIndex_ = vertexIndex_ + OHOS::TWO_STEP;
        if (vertexIndex_ == OHOS::TWO_STEP) {
...
private:
    ...
    unsigned numberVertices_;
    float arrayVertices_[BEZIER_ARC_VERTEX_NUM];
    unsigned currentCommand_;
};
...
public:
    ...
    unsigned Vertex(float* x, float* y)
    {
        return bezierArcModel_.Vertex(x, y);
    }
