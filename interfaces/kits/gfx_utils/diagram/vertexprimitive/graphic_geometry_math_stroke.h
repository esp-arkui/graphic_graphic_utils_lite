...
#define CALC_JOIN_DALT \
    ...
    LineJoinEnum _lineJoinEnum = lineJoinEnum_; \
...
class GraphicGeometryMathStroke {
public:
    using CoordType = typename VertexConsumer::ValueType;
    GraphicGeometryMathStroke()
        : strokeWidth_(OHOS::ALPHAHALF),
        strokeWidthUsingAbs_(OHOS::ALPHAHALF),
        strokeWidthPercentDivision_(OHOS::ALPHAHALF / BUF_SIZE),
        strokeWidthSignal_(1),
#if GRAPHIC_ENABLE_LINECAP_FLAG
        lineCapEnum_(BUTT_CAP),
#endif
#if GRAPHIC_ENABLE_LINEJOIN_FLAG

        miterLimitMeasure_(OHOS::DEFAULTMITERLIMIT),
        lineJoinEnum_(MITER_JOIN),
#endif
    ...
    void CalcCap(VertexConsumer& vertexConsumer, const VertexDist& vd0, const VertexDist& vd1, float len)
    {
        ...
        if (lineCapEnum_ != ROUND_CAP) {
            if (lineCapEnum_ == SQUARE_CAP) {
            ...
            switch (_lineJoinEnum) {
                ...
                case MITER_JOIN_ROUND:
                    CalcMiter(vertexConsumer, vertexDistBegin, vertexDistMiddle,
                              vertexDistLast, dx1, dy1, dx2, dy2, lineJoinEnum_, miterLimitMeasure_, dbevel);
                    break;
                case ROUND_JOIN:
    ...
    {
        strokeWidth_ = width * OHOS::ALPHAHALF;
        if (strokeWidth_ < 0) {
...
private:
    ...
    int strokeWidthSignal_;

    float approxScaleRadio_;
#if GRAPHIC_ENABLE_LINECAP_FLAG
    LineCapEnum lineCapEnum_;
#endif
#if GRAPHIC_ENABLE_LINEJOIN_FLAG
    LineJoinEnum lineJoinEnum_;
