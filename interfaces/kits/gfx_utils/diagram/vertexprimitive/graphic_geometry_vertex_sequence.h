 ...
 * @{
 *
 * @brief Defines GraphicGeometryVertexSequence VertexDist VertexDistCmd.
 *
 * @since 1.0
 * @version 1.0
 */
 ...
 *
 * @brief Defines Define the GraphicGeometryVertexSequence class.
 *
 * @since 1.0
 * @version 1.0
 */
template <class T, unsigned S = BLOCK_SHIFT_SIZE>
class GraphicGeometryVertexSequence : public GraphicGeometryPlainDataBlockVector<T, S> {
public:
    using BaseType = GraphicGeometryPlainDataBlockVector<T, S>;
    /**
...
void GraphicGeometryVertexSequence<T, S>::Close(bool closed)
{
...
template <class T, unsigned S>
void GraphicGeometryVertexSequence<T, S>::Add(const T& val)
{
    if (BaseType::Size() > 1) {
        if (!(*this)[BaseType::Size() - TWO_STEP]((*this)[BaseType::Size() - 1])) {
            BaseType::RemoveLast();
...
template <class T, unsigned S>
void GraphicGeometryVertexSequence<T, S>::ModifyLast(const T& val)
{
    BaseType::RemoveLast();
    Add(val);
}
...
struct VertexDistCmd : public VertexDist {
     ...
     * @version 1.0
     */
    VertexDistCmd(float x_, float y_, unsigned cmd_) : VertexDist(x_, y_), cmd(cmd_) {}
};
} // namespace OHOS
#endif
