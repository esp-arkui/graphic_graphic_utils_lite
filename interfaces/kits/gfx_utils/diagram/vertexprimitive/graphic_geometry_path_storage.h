...
public:
    ...
    VertexBlockStorage()
        : totalVertices_(0), totalBlocks_(0), maxBlocks_(0), croodBlocks_(0), cmdBlocks_(0) {}

    const VertexBlockStorage& operator=(const VertexBlockStorage& vertexBlockStorage)
    {
        RemoveAll();
        for (unsigned iIndex = 0; iIndex < vertexBlockStorage.TotalVertices(); iIndex++) {
            float x;
            float y;
    ...
    {
        float* coordPtr = 0;
        *StoragePtrs(&coordPtr) = (uint8_t)cmd;
        coordPtr[0] = float(x);
        coordPtr[1] = float(y);
        totalVertices_++;
    }
