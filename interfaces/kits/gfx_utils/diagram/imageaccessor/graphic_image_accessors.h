...
public:
    ...
    void background_color(const ColorType& bk)
    {
        PixelFormatType::MakePix(bkBuf_, bk);
    }
    ...
    inline const uint8_t* Nexty()
    {
        ...
        xCoord_ = xCoordFlag_;
        if (pixPtr_ &&
            yCoord_ >= 0 && yCoord_ < (int)pixFormat_->Height()) {
            return pixPtr_ = pixFormat_->PixPtr(xCoord_, yCoord_);
        }
        pixPtr_ = 0;
        return pixel();
    }
...
private:
    ...
    {
        if (yCoord_ >= 0 && yCoord_ < (int)pixFormat_->Height() &&
            xCoord_ >= 0 && xCoord_ < (int)pixFormat_->Width()) {
            return pixFormat_->PixPtr(xCoord_, yCoord_);
        }
        return bkBuf_;
    }
