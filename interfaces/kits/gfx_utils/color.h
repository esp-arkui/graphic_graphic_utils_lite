...
#include "gfx_utils/diagram/common/graphic_common_gamma_lut.h"
...
struct Rgba8T {
    ...
    static inline float ToFloat(uint8_t uint8_t)
    {
    ...
    static inline uint8_t Invert(uint8_t uint8_t)
    {
    ...
    template <typename T>
    static inline T Downshift(T value, unsigned digit)
    {
        return value >> digit;
    }
     ...
     * @version 1.0
     */
    inline SelfType Gradient(const SelfType& color, float k) const
    {
        SelfType ret;
