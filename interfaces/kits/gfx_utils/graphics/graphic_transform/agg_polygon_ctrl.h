...
#include <gfx_utils/graphics/graphic_geometry/agg_ellipse.h>
...
namespace agg
{
    ...
    public:
        ...
        const ColorT& color() const { return m_color; } 
...
//----------------------------------------------------------shadow_ctrl
    template<class ColorT> class shadow_ctrl : public polygon_ctrl_impl
    {
    public:
        shadow_ctrl(unsigned np, double point_radius=5) :
            polygon_ctrl_impl(np, point_radius),
            m_color(rgba(0.0, 0.0, 0.0)),
            m_offsetx(0),
            m_offsety(0),
        ...
        void fill_color(const ColorT& c) { m_color = c; }
        const ColorT& color() const { return m_color; } 
    ...
    private:
        shadow_ctrl(const shadow_ctrl<ColorT>&);
        const shadow_ctrl<ColorT>& operator = (const shadow_ctrl<ColorT>&);
        double m_offsety;
        double m_offsetx;
        double m_radius;
        bool isblur;
