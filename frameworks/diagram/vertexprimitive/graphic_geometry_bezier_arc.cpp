...
#include <gfx_utils/diagram/vertexprimitive/graphic_geometry_bezier_arc.h>

namespace OHOS {
#if GRAPHIC_ENABLE_BEZIER_ARC_FLAG
#define BEZIER_ARC_SVG_INIT \
    if (ry < 0.0f) { \
        ry = -ry; \
    } \
    if (rx < 0.0f) { \
                     ...
                     float sweepAngle)
{
    ...
    if (std::fabs(sweepAngle) < 1e-10) {
        ...
        arrayVertices_[INDEX_ZERO] = centerX + rx * std::cos(startAngle);
        arrayVertices_[INDEX_ONE] = centerY + ry * std::sin(startAngle);
        arrayVertices_[INDEX_TWO] = centerX + rx * std::cos(startAngle + sweepAngle);
        arrayVertices_[INDEX_THREE] = centerY + ry * std::sin(startAngle + sweepAngle);
        return;
    }
                        ...
                        float x2, float y2)
{
    ...
    unsigned limit = bezierArcModel_.GetNumberVertices() - BEZIER_ARC_SETUP;
